...
def readiness_range(low, high):
    return f"Projected {low}-{high} of {rules.EXAM_TOTAL_ITEMS}"


def readiness_pass_line():
...
def benchmark_report_title():
    return "By Florida standard"


def benchmark_report_absent(domain):
    if domain == 4:
        return ("Landmark cases and legislation are tracked one by one rather "
                "than by standard, so this view reports per case instead.")
    return ""
